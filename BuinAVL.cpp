/* Este programa faz busca e insercao numa AVL
 ...
 0 60 5 25 10 15 20 80 30 35 40 90 45 70 75 95 50 79 76 42 47 	  
...
void Insere_AVL (int x, arv &p, bool &h){
	if(p==NULL){
		...
		if (T == NULL)
			T=p; // se for o primeiro no faz t apontar para esse no
		h=true;  
		Imprime_AVL();
	}
    else if(x<p->chave){
...
int main(){
	T=NULL;   i = 21; //insere 21 chave
    while(i--){
		cout <<"Numero: "; cin >> x;
        if (Busca_AVL(x, T)) //nao insere
			cout<<x<<" - chave duplicada"<<endl;
		else{
			cout<<x <<" - chave inserida"<<endl; //insere
			Insere_AVL(x, T, h);
		}
	}
	cin.get();
